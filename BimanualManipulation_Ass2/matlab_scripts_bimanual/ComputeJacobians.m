...
if (mission.phase == 2)
    ...
    % 
    % trasformata giusta è wTt 2dal 
    pandaArm.ArmL.tSo = [eye(3) zeros(3,3); (skew(pandaArm.ArmL.wTt(1:3,1:3) * (pandaArm.ArmL.tDo)))' eye(3)]; % rigid jacobian bethwwe ee and tool
    pandaArm.ArmR.tSo = [eye(3) zeros(3,3); (skew(pandaArm.ArmR.wTt(1:3,1:3) * (pandaArm.ArmR.tDo)))' eye(3)];
