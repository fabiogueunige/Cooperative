...
[pandaArm2, goal] = InitRobot(model,wTb_right);
...
for t = 0:deltat:end_time
    ...
    [pandaArm1] = ComputeTaskReferences(pandaArm1,mission, goal);
    [pandaArm2] = ComputeTaskReferences(pandaArm2,mission, goal);
    ...
    [pandaArm1,pandaArm2,mission, goal] = UpdateMissionPhase(pandaArm1,pandaArm2,mission, goal);
    ...
    loop = loop + 1;
    if (mission.phase == 2)
        distance_x(loop) = pandaArm1.wTt(1, 4) - pandaArm2.wTt(1, 4);
        traiettoria_L(:, loop) = pandaArm1.wTo(1:3, 4);
        traiettoria_R(:, loop) = pandaArm2.wTo(1:3,4);
        los_point (:, loop) = goal.los(1:3, 4);
        [lin ang] = CartError(goal.los, pandaArm1.wTo);
        lineare(:, loop) = lin;
        angolare(: ,loop) = ang;
    end
